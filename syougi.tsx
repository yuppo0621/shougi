...
const appId = typeof __app_id !== 'undefined' ? __app_id : 'default-shogi-app';
...
// --- Shogi Constants & Engine Definitions ---
const BOARD_SIZE = 9;
...
// Generates pseudo-legal moves (doesn't check if king is left in check)
const getPseudoLegalMoves = (board, hands, player) => {
  ...
  for (let type of uniqueHandPieces) {
    for (let y = 0; y < 9; y++) {
      // Rule: Cannot drop where piece can never move
      if (type === 'FU' || type === 'KY') {
...
// Check if a player's king is currently under attack
const isCheck = (board, player) => {
  ...
  if (!kingPos) return true; // King captured (shouldn't happen in valid states)

  const opponent = player === 'black' ? 'white' : 'black';
  const oppMoves = getPseudoLegalMoves(board, { black: [], white: [] }, opponent); // Hand doesn't matter for check
  
  return oppMoves.some(m => m.to.x === kingPos.x && m.to.y === kingPos.y);
};

// Get strictly legal moves (filtering out moves that result in own king being in check)
...
const applyMove = (state, move, switchTurn = true) => {
  ...
  if (from) {
    ...
    if (target) {
      ...
      // Demote if captured a promoted piece
...
// Basic static evaluation for AI
const evaluateBoard = (state, maximizingPlayer) => {
  ...
  for (let y = 0; y < 9; y++) {
    for (let x = 0; x < 9; x++) {
      ...
      if (p) {
        ...
        // Slight positional bonus for advancing
        if (p.owner === 'black') {
          score += val + (8 - y);
        } else {
          score -= val + y;
  ...
  // Hand value
  state.hands.black.forEach(p => score += PIECE_VALUES[p] * 1.1); // Slightly prefer in hand
  state.hands.white.forEach(p => score -= PIECE_VALUES[p] * 1.1);
  
  return maximizingPlayer === 'black' ? score : -score;
};

// AI calculation (Minimax with Alpha-Beta, depth varies by level)
const calculateAIMove = (state, levelIndex) => {
  // Levels: 0 (5級) to 12 (8段)
  ...
  const randomChance = Math.max(0, (10 - levelIndex) * 0.08); // 80% at lv 0, 0% at lv 10+
  if (Math.random() < randomChance) {
    return legalMoves[Math.floor(Math.random() * legalMoves.length)];
  }

  // Depth calculation: 1 for lowest, up to 3 for highest to keep UI responsive
  const maxDepth = levelIndex < 4 ? 1 : levelIndex < 9 ? 2 : 3;

  let bestMove = null;
  let bestValue = -Infinity;

  // Move ordering: put captures and promotions first to optimize alpha-beta
  legalMoves.sort((a, b) => {
  ...
  const minimax = (currentState, depth, alpha, beta, isMaximizing) => {
    ...
    if (moves.length === 0) return isMaximizing ? -99999 : 99999; // Checkmate
  ...
  for (let move of legalMoves) {
    ...
    // Add small random noise to evaluation to prevent repeating identical games
...
export default function App() {
  ...
  const [mode, setMode] = useState('menu'); // 'menu', 'cpu_select', 'lobby', 'playing'
  const [cpuLevel, setCpuLevel] = useState(0); // 0-12 index
  const [gameState, setGameState] = useState(null);
  const [selectedPos, setSelectedPos] = useState(null);
  const [validMoves, setValidMoves] = useState([]);
  const [pendingPromotion, setPendingPromotion] = useState(null); // stores move waiting for promo choice
  ...
  const startCPUGame = (levelIndex) => {
    ...
    setPlayerColor('black'); // Player is always black in CPU for now
    setMatchId(null);
    setGameState({
      board: createInitialBoard(),
      hands: { black: [], white: [] },
  ...
  const renderPiece = (piece, isFlipped) => {
    ...
    return (
      <div className={`w-full h-full flex flex-col items-center justify-center font-bold font-serif
        ...
        `} style={{
          // Simple css shape for shogi piece
            ...
            <div className="grid grid-cols-9 gap-px bg-stone-800 border-2 border-stone-800">
              ...
              {Array(9).fill(null).map((_, yIndex) => {
                ...
                return Array(9).fill(null).map((_, xIndex) => {
                  const visualX = playerColor === 'black' ? 8 - xIndex : xIndex; // Shogi convention: file 9 on left
                  ...
                  return (
                    <div 
                      ...
                      `}
                    >
                      {/* Grid Line artifacts (Subtle dots) */}
